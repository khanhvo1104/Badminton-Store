...
import { setBrandActive } from "@/features/brands/actions/set-brand-active";
import { INITIAL_BRAND_ACTIVATION_STATE } from "@/features/brands/brand-form-state";

type BrandActivationFormProps = {
  brandId: string;
  isActive: boolean;
  brandName: string;
};

export function BrandActivationForm({
  brandId,
  isActive,
  brandName,
}: BrandActivationFormProps) {
  ...
  return (
    <section
      aria-labelledby="brand-activation-heading"
      className="rounded-3xl border border-white/10 bg-slate-950/40 p-6"
    >
      <h2
        id="brand-activation-heading"
      ...
      <p className="mt-2 text-sm leading-7 text-slate-300">
        {isActive
          ? "This brand is active and can appear for public catalog users."
          : "This brand is inactive. Authorized CMS users can still see it here."}
      </p>
      ...
      <form ref={formRef} action={formAction} className="mt-4 space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          >
            {nextActive ? "Activate brand" : "Deactivate brand"}
          </button>
          <noscript>
      ...
      <ConfirmationDialog
        open={dialogOpen}
        title={nextActive ? "Activate brand?" : "Deactivate brand?"}
        description={
          nextActive
            ? `Activate “${brandName}” so it can appear for public catalog users.`
            : `Deactivate “${brandName}”? It will stay visible to CMS staff and remain hidden from public users.`
        }
